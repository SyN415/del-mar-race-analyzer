...
def scrape_smartpick_data_for_card(date_str: str, num_races: int) -> Dict:
    ...
    # Convert date format from MM/DD/YYYY to MMDDYYYY for SmartPick URLs
    date_parts = date_str.split('/')
    if len(date_parts) == 3:
        smartpick_date = f"{date_parts[0]}{date_parts[1]}{date_parts[2]}"
    else:
        smartpick_date = "09052025"  # fallback
